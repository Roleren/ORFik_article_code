...
a <- windowCoveragePlot(coverage, scoring = scores[1], title = title) + scale_x_continuous(breaks = c(50, 100)) + theme(panel.spacing.x = unit(0.2, "lines"))
aa <- windowCoveragePlot(coverage, scoring = scores[2], title = title) + scale_x_continuous(breaks = c(50, 100)) + theme(panel.spacing.x = unit(0.2, "lines"))
aaa <- windowCoveragePlot(coverage, scoring = scores[3], title = title) + scale_x_continuous(breaks = c(50, 100)) + theme(panel.spacing.x = unit(0.2, "lines"))

final <- cowplot::plot_grid(a, aa, aaa, nrow = 1, labels = "AUTO")
ggsave(pasteDir(plotFolder, paste0(df@experiment,"_cp_all_", format)),final,
